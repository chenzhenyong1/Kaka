...
{
    _zipFile = SSzipOpen( (const char*)[zipFile UTF8String], (isAppend ? APPEND_STATUS_ADDINZIP : APPEND_STATUS_CREATE) );
	if( !_zipFile ) 
		return NO;
	return YES;
}
...
//	tm_zip filetime;
	...
	{
		ret = SSzipOpenNewFileInZip( _zipFile,
	...
	else
	{
		...
		ret = SSzipOpenNewFileInZip3( _zipFile,
								  (const char*) [newname cStringUsingEncoding:self.stringEncoding],
								  &zipInfo,
								  NULL,0,
								  NULL,0,
	...
	ret = SSzipWriteInFileInZip( _zipFile, (const void*)[data bytes], dataLen);
	if( ret!=Z_OK )
	{
		return NO;
	}
	ret = SSzipCloseFileInZip( _zipFile );
	if( ret!=Z_OK )
		return NO;
	return YES;
}
...
-(BOOL) CloseZipFile2
{
	...
	if( _zipFile==NULL )
		return NO;
	BOOL ret =  SSzipClose( _zipFile,NULL )==Z_OK?YES:NO;
	_zipFile = NULL;
	return ret;
}
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	_unzFile = SSunzOpen( (const char*)[zipFile UTF8String] );
	if( _unzFile )
	{
		unz_global_info  globalInfo = {0};
		if( SSunzGetGlobalInfo(_unzFile, &globalInfo )==UNZ_OK )
		{
...
-(BOOL) UnzipFileTo:(NSString*) path overWrite:(BOOL) overwrite
{
    ...
    int index = 0;
    int progress = -1;
	int ret = SSunzGoToFirstFile( _unzFile );
	...
	do{
        @autoreleasepool {
            if( [_password length]==0 )
                ret = SSunzOpenCurrentFile( _unzFile );
            else
                ret = SSunzOpenCurrentFilePassword( _unzFile, password );
            if( ret!=UNZ_OK )
            {
            ...
            ret = SSunzGetCurrentFileInfo(_unzFile, &fileInfo, NULL, 0, NULL, 0, NULL, 0);
            if( ret!=UNZ_OK )
            {
                [self OutputErrorMessage:@"Error occurs while getting file info"];
                success = NO;
                SSunzCloseCurrentFile( _unzFile );
                break;
            }
            char* filename = (char*) malloc( fileInfo.size_filename +1 );
            SSunzGetCurrentFileInfo(_unzFile, &fileInfo, filename, fileInfo.size_filename + 1, NULL, 0, NULL, 0);
            ...
            do
            {
                read = SSunzReadCurrentFile(_unzFile, buffer, 4096);
                if (read >= 0)
                {
                    if (fp == NULL) {
            ...
            if (ret == UNZ_OK) {
                ret = SSunzCloseCurrentFile( _unzFile );
                if (ret != UNZ_OK) {
            ...
            if (ret == UNZ_OK) {
                ret = SSunzGoToNextFile( _unzFile );
            }
