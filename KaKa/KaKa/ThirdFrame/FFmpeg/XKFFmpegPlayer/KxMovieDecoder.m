...
- (kxMovieError) openInput: (NSString *) path
{
    ...
    //设置加载时间
//    av_dict_set(&opts, "analyzeduration", "2000000", 0); //解析的最大时长这里的数字代表微妙 2000000/1000000 = 2s
//    av_dict_set(&opts, "probesize", "1331330", 0);        //解析的容量上限为122880/1024M = 120M 可以自己设置不能太小否则会导致流的信息分析不完整
    ...
    if ([[path lowercaseString] containsString:@"live"]) {
        // avformat_find_stream_info花费较长的时间，造成刚刚开始播放就出现延迟；如果是直播，probesize2调小一点，不然有延迟
        // probesize2值太小，分辨率大的视频有可能播放不出来
        formatCtx->probesize2 = 2*1024;
    }
    
//    formatCtx->max_analyze_duration2 = 2 * AV_TIME_BASE;
...
{    
    ...
    if (!_outputWidth) {
        _outputWidth = (int)self.frameWidth;
    }
    
    if (!_outputHeight) {
        _outputHeight = (int)self.frameHeight;
    }
...
- (BOOL) setupScaler
{
    ...
    _swsContext = sws_getContext(_videoCodecCtx->width,
...
- (KxVideoFrame *) handleVideoFrame
{
    ...
    if (_videoFrameFormat == KxVideoFrameFormatYUV) {
        ...
        rgbFrame.rgb = [NSData dataWithBytes:_picture.data[0]
                                    length:rgbFrame.linesize * _outputHeight];
        frame = rgbFrame;
    }    
    
    frame.width = _outputWidth;
    frame.height = _outputHeight;
