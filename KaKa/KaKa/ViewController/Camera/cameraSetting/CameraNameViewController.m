...
-(void)socketWithBody:(NSString *)body
{
    if (body.length == 0)
    {
        [self addActityText:@"请输入名称" deleyTime:1];
        return;
    }
    ...
    msg.msgBody = [NSString stringWithFormat:@"cdrSystemCfg.name=\"%@\"",body];
    [socketManager sendData:msg receiveData:^(MsgModel *msg) {
        
        MMLog(@"aaa");
        if ([msg.msgBody isEqualToString:@"OK"])
        {
            [UserDefaults setObject:body forKey:[NSString stringWithFormat:@"CameraUserName_%@",[SettingConfig shareInstance].currentCameraModel.macAddress]];
...
- (void)addCamereNameTF {
    ...
    _cameraNameTF = [self textFieldWithFrame:CGRectMake(0, 16, SCREEN_WIDTH, 50) sizeFont:FONTCALE_Y * 30 background:nil keyBoardType:UIKeyboardTypeDefault placeholder:@"请输入摄像机名称" placeholdFont:FONTCALE_Y * 30 placehodlColor:[UIColor grayColor] secure:NO inView:self.view];
    ...
    UILabel *leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, 65, 50)];
    leftLabel.text = @"摄像机名";
    leftLabel.font = [UIFont systemFontOfSize:FONTCALE_Y * 30];
