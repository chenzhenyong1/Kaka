...
- (void)addTitleLabel {
    [self labelWithFrame:CGRectMake(0, 151 * PSDSCALE_Y, SCREEN_WIDTH, 57 * PSDSCALE_Y) inView:self.view textColor:RGBSTRING(@"333333") fontSize:45 * FONTCALE_Y text:@"使用手机号找回密码" alignment:NSTextAlignmentCenter bold:YES fit:NO];
}
...
- (void)addInputView {
    ...
    NSArray *leftTextsArray = @[@" +86", @" 验证码", @" 新密码"];
    NSArray *placeholdersArray = @[@"请填写手机号码", @"请填写验证码", @"请填写新密码 "];
    ...
    // 提交按钮
    [self buttonWithFrame:CGRectMake(60 * PSDSCALE_X, 750 * PSDSCALE_Y, SCREEN_WIDTH - 2 * 60 * PSDSCALE_X, 97 * PSDSCALE_X) inView:self.view title:@"提 交" titleColorNormal:WHITE_COLOR titleColorSelected:CLEARCOLOR titleFontSize:35 * FONTCALE_Y backgroundNormal:GETNCIMAGE(@"login_btn_bg.png") backgroundSelected:nil cornerRadius:0 borderWidth:0 borderColor:0 block:^(UIButton *sender) {
        [weakSelf commitBtnClicked];
    }];
}
...
- (void)commitBtnClicked {
    ...
    if (_pwdTF.text.length < 6 || _pwdTF.text.length > 15) {
        [self addActityText:@"请输入6到15位新密码" deleyTime:1];
        return;
    }
...
- (void)resetPwd {
    ...
    [RequestManager updatePasswordWithAuthToken:FORMATSTRING(_authToken) newPassword:_pwdTF.text loginToken:nil succeed:^(id responseObject) {
        
        [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
        [self resolveReturnData:responseObject ok_block:^(NSDictionary *resultDic) {
            [self addActityText:@"密码修改成功" deleyTime:1];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self.navigationController popViewControllerAnimated:YES];
            });
        } err_block:^(NSDictionary *resultDic) {
            [self addActityText:@"密码修改失败" deleyTime:1];
        }];
