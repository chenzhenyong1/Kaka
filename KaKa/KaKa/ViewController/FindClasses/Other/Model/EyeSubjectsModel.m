...
@implementation EyeSubjectsModel
{
    CGFloat _cellHeight;
}
...
- (CGFloat)cellHeight
{
    ...
    if (!_cellHeight) {
        ...
        // 计算文字的高度
        CGFloat textH = [self.title boundingRectWithSize:maxSize options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]} context:nil].size.height;
        
//        ZYLog(@"textH = %f",textH);
        
        // cell的高度
        if ([self.title isEqualToString:@" "]) {
            _cellHeight = 10 + 40 + 10 + kScreenWidth * 9/16 + 10 + 10 + TABBARHEIGHT;
        }else{
            _cellHeight = 10 + 40 + 10 + kScreenWidth * 9/16 + 10 + textH + 15 + TABBARHEIGHT;
        }
    }
    return _cellHeight;
}
